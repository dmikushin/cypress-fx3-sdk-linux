...
/*
   ...
   and 1024 for super speed. CY_FX_GPIFTOUSB_DMA_BUF_COUNT in the header file defines the
...
volatile CyBool_t glChannelSuspended = CyFalse;
    ...
    if (bType == CY_U3P_USB_VENDOR_RQT)
    {
        if (bRequest == 0x76)
        {
            ...
            if (CyU3PUsbGetSpeed () != CY_U3P_SUPER_SPEED)
            {
                ...
                CyU3PDmaChannelSetSuspend (&glDmaChHandle, CY_U3P_DMA_SCK_SUSP_NONE, CY_U3P_DMA_SCK_SUSP_CUR_BUF);
                while ((!glChannelSuspended) && (timeout--))
                    CyU3PThreadSleep (1);
            }
    ...
    )
{
    ...
    case CY_U3P_USB_EVENT_EP0_STAT_CPLT:
        ...
        if (CyU3PUsbGetSpeed () != CY_U3P_SUPER_SPEED)
        {
            CyU3PDmaChannelSetSuspend (&glDmaChHandle, CY_U3P_DMA_SCK_SUSP_NONE, CY_U3P_DMA_SCK_SUSP_NONE);
            CyU3PDmaChannelResume (&glDmaChHandle, CyFalse, CyTrue);
        }
        break;
