/*
## Cypress USB 3.0 Platform source file (cycx3_rgb24.c)
## ===========================
##
...
/* This application example implemnets a RGB888 video camera over the UVC 1.1 prorocol.on the CX3 using an
 * Aptina AS0260 image sensor (streaming RGB 565 stream) over Mipi CSI2.  
    ...
    if (bType == CY_U3P_USB_CLASS_RQT)
    {
        ...
        if((wIndex & 0x00FF) == CX3_UVC_STREAM_INTERFACE)
        {
            ...
            if (bRequest == CX3_USB_UVC_GET_CUR_REQ)
            {
                ...
                {
                    /* Probe Control for 320x240 stream*/
                    CyU3PMemCopy(glProbeCtrl, (uint8_t *)glQvga90ProbeCtrl, CX3_UVC_MAX_PROBE_SETTING);
                }
                else /* FULL-Speed*/
                {
                    /* Probe Control for 320x240 @5FPS stream*/
                    CyU3PMemCopy(glProbeCtrl, (uint8_t *)glQvga5ProbeCtrl, CX3_UVC_MAX_PROBE_SETTING);
                }
        ...
        else if((wIndex & 0x00FF) == CX3_UVC_CONTROL_INTERFACE) /* Video Control Interface */
        {
            ...
            if(wValue == CX3_UVC_VC_REQUEST_ERROR_CODE_CONTROL)
            {
...
CyCx3UvcApplnInit (void)
{
     ...
     */
    status = CyU3PMipicsiGpifLoad(CY_U3P_MIPICSI_BUS_24, CX3_UVC_DATA_BUF_SIZE);
    if (status != CY_U3P_SUCCESS)
    {
    ...
    /* Set default interface parameters (1080p) to the CX3 MIPI interface and leave it in Sleep*/        
    if( CyU3PUsbGetSpeed() == CY_U3P_SUPER_SPEED)
    {
        status = CyU3PMipicsiSetIntfParams(&cfgUvc1080p30NoMclk, CyFalse);
    }
